...
figure
plot(x);
title('Downsampled Original Audio at 8 kHz');
xlabel('Sample');
ylabel('Amplitude');

% beginFreq = 700 / (8000/2);
% endFreq = 3500 / (8000/2);
...
for i = 1:size(yorig,2) % buffer
    ...
    [Abit{i},Tbit(i)] = bitAllocate (1.75,n,var1{i},280,temp_mse);
     ...
     %[quant{i},bitstream{i},totalBit(i),aLevel{i}] = uniEncoder(Abit{i},g{i},var1{i},x_dct{i},src);
     [quant2{i},bitstream2{i},totalBit2(i),aLevel2{i}] = uniEncoder2(Abit{i},g{i},var1{i},x_dct{i},src);
    
		%Side info (We still haven't quantized this)
		
		si_maxDCT(i) = max(x_dct{i}); %Get max value
        si_minDCT(i) = min(x_dct{i}); % get min
...
[r_sig, r_dct,s] = uniDecoder(bitstream2,Abit,si_maxDCT,si_minDCT);

% test filter
% fc=5000; %cut-off frequency (you mentioned 1-5 Hz in your question so I used 5)
% f1 = 100/8000;
% Wn= fc/8000; % Normalized frequency
% filt_order=7; % arbitrarily chose an 8th order filter
% [b,a]=butter(filt_order,[f1,Wn], 'bandpass'); 
% % butter creates the coefficients of your digital filter
% % the optional parameter 'high' specifies a high-pass filter
% r_sig=filter(b,a,r_sig);
...
% snr_last = snr(x,err);
 plotting(215,h,h1,x_dct,var1,Abit,quant2,r_sig,x,f,acq);
% 
